...
Now, they would like you to determine the total number of findGroupInGraph.

In the example above, there were 2 findGroupInGraph: one consisting of programs 0,2,3,4,5,6,
and the other consisting solely of program 1.

How many findGroupInGraph are there in total?
...
class Graph(nodes: List<Node>) {
    val nodeMap = linkedMapOf<Int, Node>()
    init {
        nodes.forEach { nodeMap.put(it.id, it) }
    }

    // Graph and Node can not be data classes because they have a recursive structure
    override fun equals(other: Any?): Boolean = nodeMap.values.toList() == (other as Graph).nodeMap.values.toList()
    override fun hashCode(): Int = nodeMap.values.toList().hashCode()
    override fun toString(): String ="Graph(nodes=${nodeMap.values})"
}

class Node(val id: Int, var connectedTo: List<Node> = listOf()) {
    override fun equals(other: Any?): Boolean = id == (other as Node).id && connectedTo.map {it.id} == other.connectedTo.map {it.id}
    override fun hashCode(): Int = 31 * id + connectedTo.map {it.id}.hashCode()
    override fun toString(): String = "Node(id=$id, connectedTo=${connectedTo.map {it.id}})"
}

fun findGroupInGraph(graph: Graph, rootId: Int, visited: MutableSet<Int> = mutableSetOf()): List<Int> = buildSequence {
    val root = graph.nodeMap[rootId]
    if (root != null) {
        yield(root.id)
        visited.add(rootId)
        root.connectedTo.forEach {
            if (! visited.contains(it.id)) {
                findGroupInGraph(graph, it.id, visited).forEach {
                    yield(it)
                }
            }
        }
...
fun findAllGroups(graph: Graph) = with(graph) {
    val foundGroups = linkedSetOf<List<Int>>()
    this.nodeMap.values.forEach {
        val id = it.id
        val orderedGroup = findGroupInGraph(this, id).sorted()
        foundGroups.add(orderedGroup)
    }
    foundGroups.toList()
}

data class Connection(val id: Int, val connected: List<Int>)

fun parseConnections(input: String): List<Connection> =
        input.split("\n")
                .filter { !it.isBlank() }
                .map {
                    val connectionData = it.split("<->").filter { !it.isBlank() }.map { it.trim() }
                    val id = connectionData[0].toInt()
                    val connected = connectionData[1].split(",").map { it.trim().toInt() }
                    Connection(id, connected)
                }

fun createGraphFromConnections(connections: List<Connection>): Graph {
    val nodeMap = mutableMapOf<Int, Node>()
    connections.forEach { nodeMap.put(it.id, Node(it.id))}
    connections.forEach {
...
class Day12Spec : Spek({
    describe("find group") {
        on("example") {
            ...
            it("group size should be 6") {
                val result = findGroupInGraph(createGraphFromConnections(parseConnections(input)), 0)
                result.size `should equal` 6
                result `should equal` listOf(0, 2, 3, 4, 6, 5)
            }
            it("number of all findGroupInGraph should be 2") {
                val result = findAllGroups(createGraphFromConnections(parseConnections(input)))
                result.size `should equal` 2
                result `should equal` listOf(listOf(0, 2, 3, 4, 5, 6), listOf(1))
            }
        }
    }
    describe("findGroupInGraph") {
        on("empty graph") {
            val graph = Graph(listOf())
            it("should be empty") {
                findGroupInGraph(graph, 0) `should equal` listOf()
            }
        }
        on("graph with one node") {
            val node = Node(0)
            val graph = Graph(listOf(node))
            it("should be this root") {
                findGroupInGraph(graph, 0) `should equal` listOf(0)
            }
            it("should be empty for nonexisting root") {
                findGroupInGraph(graph, 1) `should equal` listOf()
            }
        }
        on("graph with two connected node") {
            val node0 = Node(0)
            val node1 = Node(1)
            node0.connectedTo = listOf(node1)
            node1.connectedTo = listOf(node0)

            val graph = Graph(listOf(node0, node1))
            it("should be these nodes") {
                findGroupInGraph(graph, 0) `should equal` listOf(0, 1)
            }
        }
        on("graph with three connected node") {
            val node0 = Node(0)
            val node1 = Node(1)
            val node2 = Node(2)
            node0.connectedTo = listOf(node1)
            node1.connectedTo = listOf(node0, node2)
            node2.connectedTo = listOf(node1)

            val graph = Graph(listOf(node0, node1, node2))
            it("should be these nodes") {
                findGroupInGraph(graph, 0) `should equal` listOf(0, 1, 2)
            }
        }
    }
    describe("parse connections") {
        on("single line with simple connection") {
            val input = "1 <-> 1"
            it("should be this simple connection") {
                parseConnections(input) `should equal` listOf(Connection(1, listOf(1)))
            }
        }
        on("single line with two connection") {
            val input = "1 <-> 1, 2"
            it("should be this simple connection") {
                parseConnections(input) `should equal` listOf(Connection(1, listOf(1, 2)))
            }
        }
    }
    describe("Graph createFromConnections") {
        on("simple connection") {
            val connections = listOf(Connection(0, listOf(0)))
            it("should create graph with one node") {
                val node = Node(0)
                node.connectedTo = listOf(node)
                val graph = Graph(listOf(node))
                val graphFromConnections = createGraphFromConnections(connections)
                graphFromConnections `should equal` graph
            }
        }
        on("complex connections") {
            val connections = listOf(
                    Connection(0, listOf(1)),
                    Connection(1, listOf(0))
            )
            it("should create graph with one node") {
                val node1 = Node(0)
                val node2 = Node(1)
                node1.connectedTo = listOf(node2)
                node2.connectedTo = listOf(node1)
                val graph = Graph(listOf(node1, node2))
                val graphFromConnections = createGraphFromConnections(connections)
                graphFromConnections `should equal` graph
            }
        }
    }
    describe("size of group 0") {
        on("input") {
            val result = findGroupInGraph(createGraphFromConnections(parseConnections(day12Input)), 0)
            println("Size of group 0: ${result.size}")
        }
    }
    describe("count all groups in graph") {
        on("input") {
            val result = findAllGroups(createGraphFromConnections(parseConnections(day12Input)))
            println("All groups: ${result.size}")
        }
    }
})
