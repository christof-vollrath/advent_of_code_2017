...
fun execute(instructions: List<Int>, postInstruction: (Int) -> Int = { it + 1 }) = buildSequence {
    val program = instructions.toMutableList()
    var pc = 0 // Program counter
    while(true) {
        if (pc >= program.size) break
        val nextPc = pc + program[pc]
        program[pc] = postInstruction(program[pc])
        yield(Pair(pc, program[pc]))
        pc = nextPc
    }
}

fun execute(instructions: String, postInstruction: (Int) -> Int = { it + 1 }) = execute(
        instructions.split("\n")
                .filter { !it.isBlank() }
                .map { Integer.parseInt(it.trim()) },
        postInstruction)

fun part2PostInstruction(pc: Int) =
    if (pc >= 3) pc - 1
    else pc + 1

class Day5Spec : Spek({
    describe("execute instructions") {
        on("empty instructions") {
            val instructions = listOf<Int>()

            it("should return empty execution") {
                execute(instructions).toList() `should equal` listOf()
            }
        }
        on("simple instructions") {
            val instructions = listOf(1)

            it("should execute one instruction") {
                execute(instructions).toList() `should equal` listOf(Pair(0, 2))
            }
        }
        on("looping instructions") {
            val instructions = listOf(1, -1)

            it("should execute one instruction") {
                execute(instructions).toList() `should equal` listOf(Pair(0, 2), Pair(1, 0), Pair(0, 3))
            }
        }
        on("example instructions") {
            val instructions = listOf(0, 3, 0, 1, -3)

            it("should execute one instruction") {
                execute(instructions).toList() `should equal` listOf(Pair(0, 1), Pair(0, 2), Pair(1, 4), Pair(4, -2), Pair(1, 5))
            }
        }
        on("example instructions as string") {
            val instructions = """
                                0
                                3
                                0
                                ...
                                -3
                                """
            it("should jump out in the right number of steps") {
                execute(instructions).count() `should equal` 5
            }
        }
    }

    describe("How many steps") {
        println("Steps: ${execute(day5Input).count()}")
    }

    describe("execute instructions - part 2") {
        on("example instructions") {
            val instructions = listOf(0, 3, 0, 1, -3)
            it("should jump out in the right number of steps") {
                execute(instructions, ::part2PostInstruction).count() `should equal` 10
            }
        }
    }
    describe("How many steps - part 2") {
        println("Steps: ${execute(day5Input, ::part2PostInstruction).count()}")
    }
