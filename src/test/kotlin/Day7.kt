...
data class Program(val name: String, val weight: Int, val subprograms: List<String> = listOf())

fun findBottom(programs: List<Program>) = with(programs) {
    val candidates = this.map { it.name }.toMutableSet()
    this.forEach {
        it.subprograms.forEach { candidates.remove(it) }
    }
    candidates.first()
}
...
fun convert2Tree(name: String, programMap: Map<String, Program>): Tree = with(programMap[name]) {
    if (this == null) throw IllegalArgumentException("Program $name not found")
    val subTrees = this.subprograms.map { convert2Tree(it, programMap ) }
    Tree(this.name, this.weight, subTrees)
}

fun sumWeight(tree: Tree): Int = tree.weight + tree.subtrees.map { sumWeight(it) }.sum()

fun parseInput(input: String) = input.split("\n").filter { !it.isBlank() } .map { parseLine(it.trim()) }

fun parseLine(line: String) = with(line.split("->")) {
    val progInfo = this[0].split("""\s""".toRegex())
    val name = progInfo[0]
    val weightPart = progInfo[1]
    val weight = Integer.parseInt(weightPart.substring(1, weightPart.length-1))

    if (this.size > 1) {
        val subprograms = this[1].split(",").map { it.trim() }
...
class Day7Spec : Spek({
    describe("find bottom program") {
        on("given one tower") {
            it("should be the bottom program") {
                val programs = listOf(Program("tknk", 41))

                findBottom(programs) `should equal` "tknk"
            }
        }
        on("given one tower and a subprogram") {
            val programs = listOf(
                    Program("ugml", 68),
                    Program("tknk", 41, listOf("ugml")))

            it("should have the tower as the bottom program") {
                findBottom(programs) `should equal` "tknk"
            }
        }
        on("given a complexer structure") {
            val programs = listOf(
                    Program("ugml", 68),
                    Program("tknk", 41, listOf("ugml", "pdax")),
                    Program("padx", 45, listOf("ugml"))
            )
    ...
    describe("parse program list") {
        on("one leave line") {
            val input = "pbga (66)"

            it("should be parsed to one program") {
                parseInput(input) `should equal` listOf(Program("pbga", 66))
            }
        }
        on("two leave line") {
            val input = """
            ...
            it("should be parsed to one program") {
                parseInput(input) `should equal` listOf(
                        Program("pbga", 66),
                        Program("tknk", 41)
                )
            }
        }
        on("bottom program with subprogram") {
            val input = """
                    pbga (66)
                    tknk (41) -> pbga
                """
            ...
            it("should be parsed to one program") {
                parseInput(input) `should equal` listOf(
                        Program("pbga", 66),
                        Program("tknk", 41, listOf("pbga"))
                )
            }
        }
    }

    describe("find bottom program for example") {
        on("example") {
            val input =
                    """
                    pbga (66)
                    ...
                    cntj (57)
                """
            it("should return tknk as root") {
                findBottom(parseInput(input)) `should equal` "tknk"
            }
        }
    }
    ...
    describe("find unbalanced programs in tree") {
        on("one node is always balanced") {
        ...
        on("simple tree with unbalanced subnodes") {
            val tree = Tree("a", 1, listOf(Tree("b1", 2), Tree("b2", 3)))

            it("should result the subnodes") {
                findUnbalanced(tree) `should equal` listOf(Triple("b1", 2, 2), Triple("b2", 3, 3))
            }
        }
        on("deeper tree with unbalanced subnodes") {
            val tree = Tree("a", 1, listOf(
                    Tree("b1", 2, listOf(Tree("c11", 1))),
                    Tree("b2", 2, listOf(Tree("c21", 2)))))

            it("should result the subnodes with total weights") {
                findUnbalanced(tree) `should equal` listOf(Triple("b1", 2, 3), Triple("b2", 2, 4))
            }
        }
    }

    describe("convert program list to tree") {
        on("given one tower") {
            val programs = listOf(Program("tknk", 41))

            it("should become a tree with one node") {
                convert2Tree(programs) `should equal` Tree("tknk", 41)
            }
        }
        on("given one tower and a subprogram") {
            val programs = listOf(
            ...
            it("should become the corresponding tree") {
                convert2Tree(programs) `should equal` Tree("tknk", 41, listOf(
                                Tree("ugml", 68)
                            )
                        )
            }
    ...
    describe("find unbalanced program in example") {
        on("example") {
            val input =
                    """
                    pbga (66)
                ...
                """
            it("should return tknk as root") {
                findUnbalanced(convert2Tree(parseInput(input))) `should equal` listOf(
                        Triple("ugml", 68, 251),
                        Triple("padx", 45, 243),
                        Triple("fwft", 72, 243))
            }
        }
        describe("find unbalanced program in corrected example") {
            on("example") {
                val input =
                        """
                    pbga (66)
