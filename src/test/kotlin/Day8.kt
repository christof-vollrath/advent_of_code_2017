...
whether to increase or decrease that register's i,
...
After this process, the largest i in any register is 1.
...
What is the largest i in any register after completing the instructions in your puzzle input?

--- Part Two ---

To be safe, the CPU also needs to know the highest i held in any register during this process
so that it can decide how much memory to allocate to these operations.
For example, in the above instructions,
the highest i ever held was 10 (in register c after the third instruction was evaluated).
...
data class GreaterEqual(override val register: String, override val i: Int) : Condition() {
    override fun evaluate(registerValue: Int) = registerValue >= i
}
data class Less(override val register: String, override val i: Int) : Condition() {
    override fun evaluate(registerValue: Int) = registerValue < i
}
data class LessEqual(override val register: String, override val i: Int) : Condition() {
    override fun evaluate(registerValue: Int) = registerValue <= i
}
data class Equal(override val register: String, override val i: Int) : Condition() {
    override fun evaluate(registerValue: Int) = registerValue == i
}
data class NotEqual(override val register: String, override val i: Int) : Condition() {
    override fun evaluate(registerValue: Int) = registerValue != i
}
...
sealed class Instruction {
    ...
    fun execute(registers: MutableMap<String, Int>): MutableMap<String, Int> = registers.apply {
...
data class IncrInstruction(override val register: String, override val i: Int, override val condition: Condition) : Instruction() {
    override fun calculate(registerValue: Int) = registerValue + i
}
data class DecrInstruction(override val register: String, override val i: Int, override val condition: Condition) : Instruction() {
    override fun calculate(registerValue: Int) = registerValue - i
}

fun parseInstruction(instStr: String) = with(instStr.split("""\s+""".toRegex())) {
    val instrRegister = this[0]
    val instrCode = this[1]
    val instrValue = this[2].toInt()
    val condRegister = this[4]
    val condCode = this[5]
    val condValue = this[6].toInt()

    val condition = when(condCode) {
        ">" -> Greater(condRegister, condValue)
        ">=" -> GreaterEqual(condRegister, condValue)
        "<" -> Less(condRegister, condValue)
        "<=" -> LessEqual(condRegister, condValue)
        "==" -> Equal(condRegister, condValue)
        "!=" -> NotEqual(condRegister, condValue)
        else -> throw IllegalArgumentException("Unkown condition $condCode")
    }

    when(instrCode) {
        "inc" -> IncrInstruction(instrRegister, instrValue, condition)
        "dec" -> DecrInstruction(instrRegister, instrValue, condition)
        else -> throw IllegalArgumentException("Unkown instruction $instrCode")
    }
}

fun parseInstructions(instructions: String) = instructions.split("\n")
        .filter { !it.isBlank() }
        .map { parseInstruction(it.trim()) }

class Day8Spec : Spek({
    describe("evaluate condition") {
        on("given a condition a > 1") {
            it("should be true for a = 2") {
                val registers = mapOf("a" to 2)

                Greater("a", 1).evaluate(registers) `should equal` true
            }
            it("should be false for non existing a") {
                Greater("a", 2).evaluate(mapOf()) `should equal` false
            }
            it("should be false for a = 1") {
                val registers = mapOf("a" to 1)

                Greater("a", 1).evaluate(registers) `should equal` false
            }
        }
    }
    describe("evaluate instruction") {
        on("b inc 5 if a > 1, b = null, a = 2") {
            val instruction = IncrInstruction("b", 5, Greater("a", 1))
            val registers = mutableMapOf("a" to 2)

            it("b = 5") {
                instruction.execute(registers) `should equal` mapOf("b" to 5, "a" to 2)
            }
        }
        on("b inc 5 if a > 1, b = 1, a = 2") {
            val instruction = IncrInstruction("b", 5, Greater("a", 1))
            val registers = mutableMapOf("b" to 1, "a" to 2)

            it("b = 6") {
                instruction.execute(registers) `should equal` mapOf("b" to 6, "a" to 2)
            }
        }
        on("b inc 5 if a > 1, b = 1, a = 1") {
            val instruction = IncrInstruction("b", 5, Greater("a", 1))
            val registers = mutableMapOf("b" to 1, "a" to 1)

            it("b = 5") {
                instruction.execute(registers) `should equal` mapOf("b" to 1, "a" to 1)
            }
        }
    }
    describe("all instructions and expressions") {

        val testData = arrayOf(
                //         instruction                                                                           result in register a
                //--|-------------------------------------------------------------------------------------------|------------------------
                data(IncrInstruction("a", 7, Greater("b", 1)) as Instruction,        17),
                data(IncrInstruction("a", 7, Greater("b", 200)) as Instruction,      10),
                data(IncrInstruction("a", 7, Less("b", 200)) as Instruction,         17),
                data(IncrInstruction("a", 7, Less("b", 1)) as Instruction,           10),
                data(IncrInstruction("a", 7, Equal("b", 100)) as Instruction,        17),
                data(IncrInstruction("a", 7, Equal("b", 1)) as Instruction,          10),
                data(IncrInstruction("a", 7, NotEqual("b", 1)) as Instruction,       17),
                data(IncrInstruction("a", 7, NotEqual("b", 100)) as Instruction,     10),
                data(IncrInstruction("a", 7, GreaterEqual("b", 1)) as Instruction,   17),
                data(IncrInstruction("a", 7, GreaterEqual("b", 100)) as Instruction, 17),
                data(IncrInstruction("a", 7, GreaterEqual("b", 200)) as Instruction, 10),
                data(IncrInstruction("a", 7, LessEqual("b", 200)) as Instruction,    17),
                data(IncrInstruction("a", 7, LessEqual("b", 100)) as Instruction,    17),
                data(IncrInstruction("a", 7, LessEqual("b", 1)) as Instruction,      10),
                data(DecrInstruction("a", 7, Greater("b", 1)) as Instruction,         3)
        )

        onData("execute instruction %s", with = *testData) { instruction, expected ->
            val registers = mutableMapOf("a" to 10, "b" to 100)
            instruction.execute(registers)

            it("returns $expected") {
                 registers["a"] `should equal` expected
            }
        }
    }
    ...
    describe("parse instructions") {
        on("b inc 5 if a > 1") {
            val instStr = "b inc 5 if a > 1"

            it("should parse corresponding instruction") {
                parseInstruction(instStr) `should equal` IncrInstruction("b", 5, Greater("a", 1))
            }
        }
    }
    describe("all instructions and expressions parsed") {

        val testData = arrayOf(
                //         instruction          result in register a
                //--|-------------------------|------------------------
                data("a inc 7 if b > 1",    17),
                data("a inc 7 if b > 200",  10),
                data("a inc 7 if b < 200",  17),
                data("a inc 7 if b < 1",    10),
                data("a inc 7 if b == 100", 17),
                data("a inc 7 if b == 1",   10),
                data("a inc 7 if b != 1",   17),
                data("a inc 7 if b != 100", 10),
                data("a inc 7 if b >= 1",   17),
                data("a inc 7 if b >= 100", 17),
                data("a inc 7 if b >= 200", 10),
                data("a inc 7 if b <= 200", 17),
                data("a inc 7 if b <= 100", 17),
                data("a inc 7 if b <= 1",   10),
                data("a dec 7 if b > 1",    3)
        )

        onData("execute instruction %s", with = *testData) { instructionStr, expected ->
            val registers = mutableMapOf("a" to 10, "b" to 100)
            parseInstruction(instructionStr).execute(registers)
    ...
    describe("execute example") {
        on("example instructions") {
            ...
            parseInstructions(instructions).forEach { it.execute(registers) }

            it("registers must have correct values") {
                registers `should equal` mapOf(
                        "a" to 1,
                        "c" to -10
                )
            }
            it("max register should be 1") {
                registers.values.max() `should equal` 1
            }
        }
    }

    describe("execute example and collect highest i") {
        on("example instructions") {
            val instructions = """
                b inc 5 if a > 1
                a inc 1 if b < 5
                ...
                """
            val registers = mutableMapOf<String, Int>()
            val totalMax = parseInstructions(instructions)
                    .map { it.execute(registers).values.max() ?: 0 }
                    .max()
            ...
            it("max register should be 10") {
                totalMax `should equal` 10
            }
        }
    }
    ...
    describe("execute exercise instructions") {
        val registers = mutableMapOf<String, Int>()
        val totalMax = parseInstructions(day8Input)
                .map { it.execute(registers).values.max() ?: 0 }
                .max()
        println("max: ${registers.values.max()}")
