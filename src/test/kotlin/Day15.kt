...
T
his change makes the generators much slower, and the judge is getting impatient;
...
class Day15Spec : Spek({
    describe("count matching pairs") {
        on("example generators") {
            ...
            it("should be the right value") {
                countMatchingPairs(generatorA, generatorB, 40_000_000) `should equal` 588
            }
        }
    }
    ...
    describe("count matching pairs for exercise") {
        on("exercise generators") {
            ...
            println("Matching pairs: ${countMatchingPairs(generatorA, generatorB, 40_000_000)}")
        }
    }
    describe("count matching pairs - part 2") {
        on("example generators - part 2") {
            val generatorA = Generator(65L, 16807L, { it % 4 == 0L })
            val generatorB = Generator(8921L, 48271L, { it % 8 == 0L })

            it("should be the right value") {
                countMatchingPairs(generatorA, generatorB, 5_000_000) `should equal` 309
            }
        }
    }
    describe("count matching pairs for exercise - part 2") {
        on("exercise generators - part 2") {
            val generatorA = Generator(634L, 16807L, { it % 4 == 0L })
            val generatorB = Generator(301L, 48271L, { it % 8 == 0L })

            println("Matching pairs - part 2: ${countMatchingPairs(generatorA, generatorB, 5_000_000)}")
        }
    }
})

fun countMatchingPairs(generatorA: Generator, generatorB: Generator, nrPairs: Int) =
        buildSequence {
            repeat(nrPairs) { yield(Pair(generatorA.next(), generatorB.next())) }
        }
        .filter { lowest16Bits(it.first) == lowest16Bits(it.second)}.count()

fun lowest16Bits(value: Long) = value.toChar().toInt()

class Generator(seed: Long, val factor:  Long, val myFilter: (Long) -> Boolean = { true }) {
    var current: Long
    init {
        current = seed
    }
