...
enum class Direction { N, NE, SE, S, SW, NW, X }

fun Pair<Int, Int>.move(dir: Direction): Pair<Int, Int> {
    val deltaX = when (dir) {
        Direction.NE, Direction.SE  -> 2
        Direction.NW, Direction.SW  -> -2
        Direction.N, Direction.S, Direction.X  -> 0
    }
    val deltaY = when (dir) {
        Direction.NW, Direction.NE  -> 1
        Direction.N  -> 2
        Direction.SW, Direction.SE  -> -1
        Direction.S  -> -2
        Direction.X  -> 0
    }
    return Pair(this.first + deltaX, this.second + deltaY)
}
fun Pair<Int, Int>.move(moves: List<Direction>): Pair<Int, Int> = moves.fold(this) { pos, dir -> pos.move(dir) }

private fun Pair<Int, Int>.moveWithAllInterimPositions(moves: List<Direction>) = buildSequence {
    var currentPos = this@moveWithAllInterimPositions
    moves.forEach {
        currentPos = currentPos.move(it)
        yield(currentPos)
    }
}.toList()

fun optimalStep(from: Pair<Int, Int>, to: Pair<Int, Int>): Direction {
    val deltaX = to.first - from.first
    val absDeltaX = abs(deltaX)
    val deltaY = to.second - from.second
    val absDeltaY = abs(deltaY)
    return  if (deltaX == 0 && deltaY == 0) Direction.X
            else if (absDeltaX > absDeltaY) {
                if (deltaX > 0)
                    if (deltaY >= 0) Direction.NE
                    else Direction.SE
                else
                    if (deltaY >= 0) Direction.NW
                    else Direction.SW
            } else { // absDeltaX <= absDeltaY
                if (deltaY > 0) Direction.N
                else Direction.S
            }
}

fun optimalSteps(from: Pair<Int, Int>, to: Pair<Int, Int>) = buildSequence {
    var currentPos = from
    while (true) {
        val step = optimalStep(currentPos, to)
        if (step == Direction.X) break // Goal reached
        yield(step)
        currentPos = currentPos.move(step)
    }
}.toList()

fun parseSteps(input: String) = input
        .split(",")
...
class Day11Spec : Spek({
    describe("all moves") {
        val testData = arrayOf(
                //     direction    result
                //--|------------|---------------
                data(Direction.N, Pair(0, 2)),
                data(Direction.NE,Pair(2, 1)),
                data(Direction.SE,Pair(2, -1)),
                data(Direction.S, Pair(0, -2)),
                data(Direction.SW,Pair(-2, -1)),
                data(Direction.NW,Pair(-2, 1)),
                data(Direction.X,Pair(0, 0))
        )
        onData("input %s", with = *testData) { input, expected ->
            it("returns $expected") {
                Pair(0, 0).move(input) `should equal` expected
            }
        }
    }
    describe("combine moves") {
        on("NE,SE,NE from (0,0)") {
            val moves = listOf(Direction.NE, Direction.SE, Direction.NE)
            it("should move to (6,1)") {
                Pair(0, 0).move(moves) `should equal` Pair(6, 1)
            }
        }
        on("SE, SW, SE, SW, SW from (0,0)") {
            val moves = listOf(Direction.SE, Direction.SW, Direction.SE, Direction.SW, Direction.SW)
            it("should move to (-1,-5)") {
                Pair(0, 0).move(moves) `should equal` Pair(-2, -5)
            }
        }
    }
    describe("optimal step leading to goal") {
        on("from (0,0) to (6,0)") {
            val from = Pair(0, 0)
            val to = Pair(6, 0)
            it("should move NE") {
                optimalStep(from, to) `should equal` Direction.NE
            }
        }
    }
    /*
     ...
     */
    describe("optimal step to all directions") {
        val testData = arrayOf(
                //     to          direction
                //--|------------|---------------
                data(Pair(-2, 1), Direction.NW),
                data(Pair( 0, 2), Direction.N),
                data(Pair( 2, 1), Direction.NE),
                data(Pair( 0, 0), Direction.X),
                data(Pair(-2,-1), Direction.SW),
                data(Pair( 0,-2), Direction.S),
                data(Pair( 2,-1), Direction.SE)
        )
        onData("input %s", with = *testData) { input, expected ->
            it("returns $expected") {
                optimalStep(Pair(0, 0), input) `should equal` expected
            }
        }
    }
    describe("optimal steps leading to goal") {
        on("from (0,0) to (6,1)") {
            val from = Pair(0, 0)
            val to = Pair(6, 1)
            it("should move in 3 steps") {
                optimalSteps(from, to).size `should equal` 3
            }
        }
    }
    describe("parse steps") {
        on("string with all steps") {
            val input = "nw, n, ne, x, sw, s, se"
            it("should be parsed correctly") {
                parseSteps(input) `should equal` listOf(Direction.NW, Direction.N, Direction.NE, Direction.X, Direction.SW, Direction.S, Direction.SE)
            }
        }
    }
    ...
    describe("examples") {
        val testData = arrayOf(
                ...
                data("ne,ne,sw,sw",    0),
                data("ne,ne,s,s",      2),
                data("se,sw,se,sw,sw", 3)
        )
        onData("input %s", with = *testData) { input, expected ->
            it("returns $expected") {
                val goal = Pair(0,0).move(parseSteps(input))
                optimalSteps(Pair(0, 0), goal).size `should equal` expected
            }
        }
    }
    describe("optimal steps exercise") {
        on("input") {
            val goal = Pair(0,0).move(parseSteps(day11Input))
            println(optimalSteps(Pair(0, 0), goal).size)
        }
    }
    describe("furthest steps exercise") {
        on("input") {
            val path = Pair(0,0).moveWithAllInterimPositions(parseSteps(day11Input))
            val maxSteps = path.map { optimalSteps(Pair(0, 0), it).size }.max()
            println(maxSteps)
        }
    }
})
