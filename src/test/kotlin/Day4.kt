...
class Day4Spec : Spek({
    describe("split passphrase") {
        ...
        on("passphrase with one word") {
            ...
            it("should be list with that word") {
                splitPassphrase(passphrase) `should equal` listOf("word")
            }
        }
        on("passphrase with severa words") {
            val passphrase = "eins zwei drei"

            it("should be list with these words") {
                splitPassphrase(passphrase) `should equal` listOf("eins", "zwei", "drei")
            }
        }
    }
    ...
    describe("validate passphrase") {
        on("empty passphrase") {
            ...
            it("should not be valid") {
                validate(passphrase) `should equal` false
            }
        }
        on("passphrase with unique words") {
            val passphrase = "aa bb cc dd ee"

            it("should be valid") {
                validate(passphrase) `should equal` true
            }
        }
        on("passphrase with duplicate words") {
            val passphrase = "aa bb cc dd aa"
    ...
    describe("count valid passphrases") {
        on("empty list") {
            val passphrases = listOf<String>()
            it("should be 0") {
                countValid(passphrases) `should equal` 0
            }

        }
        on("one valid passphrase as string") {
            val passphrases = """
                aa bb cc dd ee
                """
            it("should be 1") {
                countValid(passphrases) `should equal` 1
            }
        }
        on("passphrases as string") {
            val passphrases = """
                aa bb cc dd ee
                aa bb cc dd aa
                aa bb cc dd aaa
                """
            it("should be 2") {
                countValid(passphrases) `should equal` 2
            }
        }
    }
